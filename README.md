...
| **细粒度权限控制** | 基于 RBAC 模型，支持系统管理员 / 归档管理员 / 普通用户三级角色，数据源级读写权限隔离 |
...
| 角色编码 | 角色名称 | 数据范围 | 说明 |
|----------|---------|---------|------|
| `platform_admin` | 系统管理员 | ALL | 拥有全部权限，可管理用户和数据源权限 |
| `archive_admin` | 归档管理员 | ASSIGNED | 可管理数据源权限、创建普通用户、操作被授权的归档分组 |
| `normal_user` | 普通用户 | OWN | 仅可操作被分配的归档分组，不可管理用户 |
...
系统使用 Flyway 进行数据库版本管理。当前已将所有变更整合为单一初始化脚本 `V1__init_archive_platform.sql`，全新部署直接执行该脚本即可构建完整数据库结构。

**脚本位置：** `easyarchive-starter/src/main/resources/db/migration/V1__init_archive_platform.sql`

**独立部署脚本：** `docs/database/schema.sql`（可直接在 MySQL 客户端执行）
...
| 归档核心 | `ea_archive_datasource` | 数据源表 |
| | `ea_archive_group` | 归档分组表 |
| | `ea_archive_task` | 归档任务表 |
| | `ea_archive_task_detail` | 归档任务明细表 |
| | `ea_archive_task_progress` | 归档任务进度表 |
| | `ea_archive_task_log` | 归档任务日志表 |
| | `ea_archive_group_execute_task` | 归档执行任务表 |
| | `ea_archive_group_item_by_id` | 按 ID 归档分组明细 |
| | `ea_archive_group_item_by_time` | 按时间归档分组明细 |
| 数据源授权 | `ea_archive_connection_permission` | 数据源权限表 |
| 监控告警 | `ea_monitor_rule` | 监控规则表 |
| | `ea_alert_event` | 告警事件表 |
| 站内通知 | `ea_archive_group_notification_user` | 归档分组站内通知成员 |
| | `ea_in_app_notification` | 平台站内通知主表 |
| | `ea_in_app_notification_recipient` | 平台站内通知收件箱 |
