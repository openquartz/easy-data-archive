...
CREATE TABLE IF NOT EXISTS `ea_archive_datasource` (
    ...
    `status` TINYINT NOT NULL DEFAULT 0 COMMENT '0-未测试 1-正常 2-异常 3-禁用',
...
CREATE TABLE IF NOT EXISTS `ea_archive_group` (
    ...
    `trigger_mode` VARCHAR(16) NOT NULL DEFAULT 'MANUAL' COMMENT 'MANUAL/SCHEDULE',
    `remark` VARCHAR(255) NULL COMMENT '备注',
    `created_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updated_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    `creator_id` BIGINT NULL COMMENT '创建人ID',
...
CREATE TABLE IF NOT EXISTS `ea_archive_task` (
    ...
    `current_rule_id` BIGINT NULL COMMENT '当前规则',
