...
本次直接打通当前管理界面全部已暴露的写操作，并补齐日志查询与列表能力，不引入第二套日志机制。
...
- `POST /api/v1/users/{userId}/datasource-permissions`
- `DELETE /api/v1/users/{userId}/datasource-permissions/{datasourceId}`
- `PUT /api/v1/users/{userId}/datasource-permissions`

日志要求：

- 授权类输出“为用户 X 授予数据源 Y”
- 取消授权输出“为用户 X 移除数据源 Y”
- 替换授权输出“数据源权限从 A、B 修改为 C、D”
- `biz_key` 使用 `username` 或 `username(datasource-permissions)`

### 10.6 任务操作

接口：

- `POST /api/v1/task-log/cleanup`
- `POST /api/v1/task-log/tasks/{taskId}/cancel`

日志要求：

- 清理日志记录保留天数和删除条数
- 取消任务记录任务 ID、分组标识、取消原因、取消结果
...
查询默认按 `operateTime desc, id desc` 排序。

### 11.2 界面列表页

前端新增“操作日志”列表页，并在管理员导航中暴露入口。页面要求：

- 列表展示按钮、操作人、模块、结果、时间、内容
- 顶部支持时间、操作人、模块、结果筛选
- 支持分页
- 失败记录以显眼但不过度打扰的方式展示 `errorMessage`
- 内容列支持多行展示，不截断到不可读
...
- `easyarchive-starter/src/main/java/com/openquartz/easyarchive/starter/controller/UserController.java`
