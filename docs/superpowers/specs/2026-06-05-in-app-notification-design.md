...
> 范围: 平台通用站内通知模型 + 归档分组终态通知接入 + 顶栏铃铛提醒

## 1. 目标

在现有飞书、企业微信之外，为 EasyArchive 增加一套平台内可见的站内通知能力。该能力的通知模型应尽可能通用，可供后续其他模块复用；本次首个接入场景为归档分组终态通知，并通过界面右上角铃铛入口展示。
...
虽然首个接入场景是归档分组终态通知，但通知主表、收件箱表、查询接口和铃铛组件都应避免绑定归档私有字段。归档场景特有数据应放入 `payload_json` 与扩展跳转参数中，主字段保持平台级语义，便于后续其他模块直接接入。

### 3.7 消息保留期固定为 7 天

站内通知只承担近期结果提醒，不承担长期审计归档职责。首版统一保留最近 `7` 天消息，超过保留期的主通知与收件箱记录由后台定时清理。
...
设计一条平台通用通知主记录，再为每个通知成员生成独立收件箱记录。
...
| --- | --- | --- |
| `id` | `BIGINT` | 主键 |
| `biz_type` | `VARCHAR(32)` | 业务类型，首版归档场景使用 `ARCHIVE_GROUP_TASK`，后续其他模块可扩展 |
| `biz_id` | `BIGINT` | 业务主键，按 `biz_type` 解释 |
| `group_id` | `BIGINT` | 归档分组 ID |
| `group_name` | `VARCHAR(128)` | 分组名称快照 |
| `task_id` | `BIGINT` | 任务 ID 快照 |
| `task_status` | `VARCHAR(16)` | `SUCCESS` / `FAILED` / `CANCELED` |
| `category` | `VARCHAR(32)` | 通知分类，首版建议为 `TASK_RESULT` |
| `level` | `VARCHAR(16)` | 通知级别，首版可为 `INFO` / `WARN` / `ERROR` |
...
- `biz_type` 用于区分通知来自哪个业务模块
- `biz_id` 用于回指业务对象
- `category` 用于归类通知类型，后续列表筛选可复用
- `level` 用于统一表达消息严重程度
- `title`、`content_summary`、`source_time` 是平台级展示字段
...
- 先按 `created_time` 清理 `ea_in_app_notification_recipient`
- 再清理没有任何收件箱记录且超过保留期的 `ea_in_app_notification`
- 每天凌晨执行一次

设计要求：

- 保留期不跟随归档日志保留天数，避免两个语义混用
- 首版可将 `7` 天定义为通知模块常量
- 若后续需要配置化，再单独抽到通知配置项，不在首版提前抽象
...
本方案以平台通用通知模型为核心，以任务终态事件为首个接入点，新增分组通知成员、通知主记录和用户收件箱三层模型，并配套 7 天保留期清理任务，在不干扰归档主流程的前提下，为平台提供一套真实可用、后续可复用的站内铃铛通知系统。

这套设计与现有飞书、企业微信通知形成并列关系，但不复用其渠道模型；与现有权限体系保持协作，但不耦合；与前端当前顶层布局、任务详情和工作标签页机制保持一致，并通过通用 `bizType/category/level/payload` 结构为后续其他模块复用预留稳定边界。
